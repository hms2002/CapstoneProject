...
[Serializable]
public sealed class TitleIntroSlide
{
...
#if UNITY_EDITOR
    public void EditorSet(Sprite image, string text)
    {
        this.image = image;
        this.text = text;
    }
...
[CreateAssetMenu(menuName = "Title/Title Intro Sequence", fileName = "TitleIntroSequence")]
public sealed class TitleIntroSequenceSO : ScriptableObject
{
    [Header("Slides")]
    [SerializeField] private List<TitleIntroSlide> slides = new();

    [Header("Typing")]
    [SerializeField, Min(0f)] private float secondsPerCharacter = 0.05f;

    [Header("Timing")]
    [SerializeField, Min(0f)] private float introStartFadeDuration = 1.2f;
    ...
    public IReadOnlyList<TitleIntroSlide> Slides => slides;
    public int SlideCount => slides != null ? slides.Count : 0;
    public float SecondsPerCharacter => Mathf.Max(0f, secondsPerCharacter);
    public float IntroStartFadeDuration => Mathf.Max(0f, introStartFadeDuration);
    ...
    public TitleIntroSlide GetSlide(int index)
    {
