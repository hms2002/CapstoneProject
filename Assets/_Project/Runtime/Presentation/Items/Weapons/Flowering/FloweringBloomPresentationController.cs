...
/// <summary>
/// 책임 : Flowering Bloom 컷인, 화면 테두리, 무기 reveal, 플레이어 실루엣 등 표현 전용 연출을 재생한다.
/// </summary>
public sealed class FloweringBloomPresentationController : MonoBehaviour, IFloweringBloomPresentation
{
    ...
    private static readonly FloweringBloomPresentationBackend s_playbackBackend = new();
    ...
    {
        FloweringBloomPresentationPlayback.RegisterBackend(s_playbackBackend);
    }

    /// <summary>
    /// 책임 : Flowering gameplay presentation 요청을 player root의 concrete Bloom presentation controller로 연결한다.
    /// </summary>
    private sealed class FloweringBloomPresentationBackend : IFloweringBloomPresentationBackend
    {
        public IFloweringBloomPresentation GetOrAdd(GameObject ownerObject, FloweringBloomData bloomData)
        {
            if (ownerObject == null)
                return null;

            FloweringBloomPresentationController controller =
                ownerObject.GetComponent<FloweringBloomPresentationController>();
            if (controller == null)
                controller = ownerObject.AddComponent<FloweringBloomPresentationController>();

            controller.Initialize(ownerObject, bloomData);
            return controller;
        }
    }
