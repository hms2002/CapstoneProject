...
/// <summary>
/// 책임 : 파괴된 상점이 활성화될 때 상인 포커스, 대사, 레터박스, 입력 잠금 연출을 실행한다.
/// </summary>
public sealed class MerchantActivationCinematic : MonoBehaviour
{
    ...
    private ISpeechBubblePlayback MerchantSpeechBubble => merchantSpeechBubble as ISpeechBubblePlayback;
    ...
    {
        while (UiInteractionStateQuery.HasActivePopup())
            yield return null;
    }
    ...
    private void ResolveReferences()
    {
        ...
        {
            if (activeGroup != null)
                merchantSpeechBubble = ResolveSpeechBubbleComponent(activeGroup);
            else
                merchantSpeechBubble = ResolveSpeechBubbleComponent(gameObject);
        }

        ISpeechBubblePlayback speechBubble = MerchantSpeechBubble;
        if (merchantFocusTarget == null && speechBubble != null)
            merchantFocusTarget = speechBubble.BubbleTransform != null
                ? speechBubble.BubbleTransform
                : merchantSpeechBubble.transform;
    }

    private static MonoBehaviour ResolveSpeechBubbleComponent(GameObject root)
    {
        if (root == null)
            return null;

        MonoBehaviour[] candidates = root.GetComponentsInChildren<MonoBehaviour>(includeInactive: true);
        for (int i = 0; i < candidates.Length; i++)
        {
            if (candidates[i] is ISpeechBubblePlayback)
                return candidates[i];
        }
