...
namespace UnityGAS
{
    ...
    /// </summary>
    public sealed class SpriteAfterimageEmitter2D : MonoBehaviour, IAfterimageEmitter2D
    {
        ...
        {
            AfterimageEmitterPlayback.RegisterBackend(new SpriteAfterimageEmitterBackend());
        }
        ...
        private sealed class SpriteAfterimageEmitterBackend : AfterimageEmitterPlayback.IAfterimageEmitterBackend
        {
            public IAfterimageEmitter2D Get(GameObject owner)
            {
                return owner != null ? owner.GetComponent<SpriteAfterimageEmitter2D>() : null;
            }

            public IAfterimageEmitter2D GetOrAdd(GameObject owner)
            {
                if (owner == null)
                    return null;

                SpriteAfterimageEmitter2D emitter = owner.GetComponent<SpriteAfterimageEmitter2D>();
                return emitter != null ? emitter : owner.AddComponent<SpriteAfterimageEmitter2D>();
            }

            public IAfterimageEmitter2D GetOwned(AbilityVisualRouter router, AbilitySpec spec)
            {
                return router != null ? router.GetOwnedComponent<SpriteAfterimageEmitter2D>(spec) : null;
            }

            public IAfterimageEmitter2D GetOrAddOwned(AbilityVisualRouter router, AbilitySpec spec)
            {
                if (router == null || spec == null || router.OwnerObject == null)
                    return null;

                SpriteAfterimageEmitter2D existing = router.GetOwnedComponent<SpriteAfterimageEmitter2D>(spec);
                if (existing != null)
                    return existing;

                SpriteAfterimageEmitter2D created = router.OwnerObject.AddComponent<SpriteAfterimageEmitter2D>();
                router.Register(spec, created);
                return created;
            }
        }
    }
