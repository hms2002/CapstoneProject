...
namespace UnityGAS
{
    ...
    public sealed class MotionAlignedParticleVisual2D : MonoBehaviour, IMotionAlignedParticleVisual2D
    {
        /// <summary>
        /// 책임 :
        ...
        {
            MotionAlignedParticlePlayback.RegisterBackend(new MotionAlignedParticleBackend());
        }
        ...
        private sealed class MotionAlignedParticleBackend : MotionAlignedParticlePlayback.IMotionAlignedParticleBackend
        {
            public IMotionAlignedParticleVisual2D GetOwned(AbilityVisualRouter router, AbilitySpec spec)
            {
                return router != null ? router.GetOwnedComponent<MotionAlignedParticleVisual2D>(spec) : null;
            }

            public IMotionAlignedParticleVisual2D GetOrAddOwned(AbilityVisualRouter router, AbilitySpec spec)
            {
                if (router == null || spec == null || router.OwnerObject == null)
                    return null;

                MotionAlignedParticleVisual2D existing = router.GetOwnedComponent<MotionAlignedParticleVisual2D>(spec);
                if (existing != null)
                    return existing;

                MotionAlignedParticleVisual2D created = router.OwnerObject.AddComponent<MotionAlignedParticleVisual2D>();
