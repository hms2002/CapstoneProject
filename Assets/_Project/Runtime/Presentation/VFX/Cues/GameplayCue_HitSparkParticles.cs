...
namespace UnityGAS
{
    ...
    public sealed class GameplayCue_HitSparkParticles : GameplayCueNotify
    {
        ...
        private static readonly HitSparkCuePrefabInstanceProvider s_prefabInstanceProvider = new();
        ...
        /// <summary>
        /// 책임: Core GameplayCueManager의 prefab 생성/반납 요청을 HitSpark 정적 풀 정책으로 연결한다.
        /// </summary>
        private sealed class HitSparkCuePrefabInstanceProvider : IGameplayCuePrefabInstanceProvider
        {
            public bool CanAcquire(GameObject cuePrefab)
            {
                return cuePrefab != null && cuePrefab.GetComponent<GameplayCue_HitSparkParticles>() != null;
            }

            public GameObject Acquire(GameObject cuePrefab)
            {
                return AcquireInstance(cuePrefab);
            }

            public bool TryRelease(GameObject instance, GameplayCueNotify notify)
            {
                if (instance == null || notify is not GameplayCue_HitSparkParticles)
                    return false;
            ...
            public bool ShouldSuppressAutoDestroy(GameplayCueNotify notify)
            {
                return notify is GameplayCue_HitSparkParticles;
            }
        }

        [RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.SubsystemRegistration)]
        private static void RegisterPrefabInstanceProvider()
        {
            GameplayCuePrefabInstanceProviders.Register(s_prefabInstanceProvider);
        }
        ...
        public static GameObject AcquireInstance(GameObject prefab)
        {
            ...
            GameObject resolvedPrefab = PresentationAssetPlayback.ResolvePrefab(prefab);
