...
namespace UnityGAS
{
    ...
    /// - 걷기, 허브 인트로처럼 직접 구현한 연출이더라도 Core playback contract를 통해 같은 SoundRef / GameplayTag 규칙을 재사용하게 한다.
    /// </summary>
    // Contract:
    ...
    public sealed class GameplayPresentationRuntime
    {
        ...
        private void EnsureLoop(SoundRef soundRef, in GameplayCueParams cueParams)
        {
            ...
            if (SoundPlaybackUtility.IsPlaying(activeLoopHandle))
                return;

            StopLoop();
            activeLoopHandle = SoundPlaybackUtility.Play(soundRef, BuildSoundContext(cueParams));
        }
        ...
        private void StopLoop()
        {
            ...
            SoundPlaybackUtility.Stop(activeLoopHandle);
            activeLoopHandle = AudioHandle.Invalid;
        }
