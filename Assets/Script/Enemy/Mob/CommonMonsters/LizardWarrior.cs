...
public sealed partial class LizardWarriorChargeRunner : MonoBehaviour, IMobPatternRunner, IMobPresentationCleanup
{
    ...
    private IEnumerator RunStep(LizardWarrior.ChargeContext context, LizardWarrior.ChargeStep step, AbilitySpec spec)
    {
    ...
    private void ShowWarning(Vector2 start, Vector2 direction, LizardWarrior.ChargeStep step)
    {
        if (telegraphService == null)
            return;
    ...
    private static AttackTelegraphStyle CreateWarningStyle()
    {
        ...
        style.fillColorStart = new Color(1f, 0.1f, 0f, 0.14f);
        style.fillColorEnd = new Color(1f, 0.1f, 0f, 0.34f);
        style.borderColorStart = new Color(1f, 0.5f, 0.2f, 0.95f);
        style.borderColorEnd = new Color(1f, 0.5f, 0.2f, 0.95f);
