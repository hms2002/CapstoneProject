...
for p in paths:
 ...
 children=b['911709170000000001']
 for n,x in enumerate([18,98]):
  g,tr,cr,im=[str(911709180000000000+n*10+j) for j in range(4)]
  assert 'm_IsActive: 1' in b[g],p
  assert f'm_AnchoredPosition: {{x: {x}, y: 16}}' in b[tr],p
  ...
  assert children.index(tr)<children.index('911709170000000021'),p
