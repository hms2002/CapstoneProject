...
- PlayerVisionMask and StrangeCandlestick keep their overlay masks restricted to MaskRender. Each has a matching Entity-only EntitySightMask child to reveal ShadowMonster without spanning the intervening FloatingAOE/Projectile layers (which contain locally masked weapon trails). The child inherits the sight transform and GameObject activation; charge/dropped-spear masks remain isolated in their own SortingGroups. No ProjectSettings sorting layers were added. When changing sight artwork, keep the parent/child sprite and alpha cutoff matched. Validate combined darkness/drop/charge rendering in Editor after changes.
