...
- `WeaponDetailViewV2`, legacy `WeaponDetailView`, and `EncyclopediaItemRightPage` project Skill1/Skill2 through `WeaponDefinition.GetAbility`. Only `Weapon.CrimsonBoundary` additionally displays its basic attack. Other basic attacks, including Flowering's Bloom attack and OddIron's dry fire, stay hidden in both description modes. Every `IAbilityTooltipVariantProvider` result is a separate vertical skill block; LightningSpear's two Skill1 cases no longer use stacked cards or variant-switch input.
- `ItemDetailPanel` owns the hover description mode and retained display context. New targets and reopen start simple; refreshes of the same visible target retain the selected mode. `EncyclopediaItemRightPage` owns its own mode and yields shortcut input to an active foreground hover. Reopening the book resets to simple. Neither owns gameplay state.
- `WeaponDescriptionHint` (in `WeaponDetailViewV2.cs`) projects an authored root/Image/TMP label. `GlobalUIRoot.prefab` and `EncyclopediaUI.prefab` bind the top-right hint; no runtime hint hierarchy is created. Both hints use LayoutElement.ignoreLayout and a fixed 210x45 RectTransform. The hover hint is parented to Header/Line, anchored top-left with bottom-left pivot and offset (10, 5), so it sits above the left end of the yellow separator rather than over StoryText. The book hint remains at the Header bottom-right. They contribute no row height to the content layout. Both hints retain 27-point text and a 40.5-unit glyph (1.5 times the original). Captions are `자세히 설명` / `간단히 설명` with the BackQuote key glyph; text-input focus suppresses the shortcut.
- `InputContextShortcutId.TooltipDescriptionToggle` is appended as value 3 and defaults to BackQuote. The obsolete variant shortcut retains its serialized value 2 but has no weapon UI consumer. Existing Q/E relic preview shortcuts remain unchanged.
- Weapon story copy lives in `WeaponDefinition.storyText`. `AbilityDefinition.simpleDescription` supplies the default concise body; `description` plus existing detail providers supplies detailed mode. Missing concise copy falls back to existing detail content. LightningSpear Skill1 supplies `Body` / `SimpleBody` pairs through `AbilityTooltipVariant` in `LightningSpearSkill1Data`. The current five-weapon roster (ApprenticeHeroSword, LightningSpear, CrimsonBoundary, Flowering, OddIron) has 16 authored ability summaries; unused weapon copy is unchanged. Relic effects come from `RelicLogic.BuildTooltip`, with serialized `effectTemplate` overrides taking precedence over C# defaults. Weapon-exclusive relics use `RelicDefinition.description` through `RelicLogic_WeaponExclusive`.
...
- `ChestUIManager.CompleteOpenedChest` delegates reward completion to `TreasureChest.CompleteLootSelection`, then the screen closes through the existing UI stack. Completion clears remaining rewards, keeps the opened world chest visible at 25% of its original RGB tint (alpha unchanged), freezes its animator, and disables child colliders and chest interaction. A runtime completion flag prevents reopening and repeated dimming; WorldStateChanged refreshes guidance. The unused dungeon restoration path is outside this change; current gameplay does not restore previous dungeons.
...
## Sequential acquisition presentation (2026-09-20)

- `ChestScreen.ConfirmSelection` captures display-only slot snapshots before the existing batch commit. Successful commits play in selection order: an outgoing weapon slot travels to DropZone, then the incoming full slot travels from SelectedItem to its reserved inventory slot and resizes to fit. Travel uses unscaled DOTween `InQuad` (drop, approximately 0.1333s) / `InCubic` (acquisition, approximately 0.1583s), followed by a 0.0417s arrival hold (doubled playback speed, then increased speed another 20% on 2026-09-21).
- The first outgoing slot lands at the DropZone rect center (including noncentral pivots). When a second arrives, both reflow to horizontally symmetric cells with an eight-unit canvas-space gap. Empty weapon destinations do not produce drop visuals. Duplicate relic arrivals share their reserved target and project accumulated levels in order.
- The screen-owned `AcquisitionPresentation` uses the existing chest slot prefab and read-only snapshot containers. Copies live directly under the root Canvas to avoid viewport masks. Actual source/target slots retain their parents and layout participation; temporary CanvasGroups hide them until cleanup. `ItemSlotUI.BoundContainer` identifies target views, and the bound `PlayerInventoryPanelView.DropZone` supplies the destination for shared and standalone chest screens.
- The existing EventSystem is temporarily disabled during playback, and the screen close-request gate consumes close requests. The prior EventSystem enabled state is restored on completion, commit failure or forced teardown. No new EventSystem, Canvas, serialized field or authored prefab element is required.
- Commit failure restores visibility/input without completing the chest. Normal completion and forced teardown finalize an already committed chest once, dispose tweens/copies and restore the real slots; only normal completion requests UI close. World weapon drops and persistent weapon payloads remain owned by the existing weapon batch acquisition path.
- `Tools/Validation/ChestAcquisitionPresentationRegression.cs` exercises ordered playback, paused time, drop centering/reflow, full-slot sizing, close/duplicate-confirm guards and cleanup in an isolated Unity harness. Actual prefab visual acceptance remains a separate Play Mode check.
...
- One Sword Oath dismisses the swap hint only after the slot seal, attack modifier and cooldown modifier all succeed. Rejected/rolled-back application does not complete it. Completion lasts until run end or development reset, including across hub-to-run entry.
...
### Tutorial required potion selection (2026-09-20)

`ChestScreen.FindVisibleSlot` and `TrySelectGuidedSlot` target a definition's actual offered slot rather than slot order. `RequireSelection`/`ReleaseSelectionRequirement` are owner-scoped and gate both confirm interactability and the commit path. Disable clears ownership. Ordinary chests with no owner remain unchanged. PrototypeChestNavigation retains the requirement after the first potion click, releases it on close/commit, and reacquires on reopen. Inventory inspection uses the existing inspection-only owner API; potion-use success remains owned by PlayerConsumableInventory. See TutorialSupportStructure.md for the full stage 4–7 flow.

## Drag icon coordinate projection (2026-09-21)

- `Runtime/UI/Inventory/Common/DragIcon.cs` caches its owning Canvas on Awake and reads the current root Canvas render mode/camera on every Follow. Screen-space pointer positions are projected onto the parent RectTransform plane with RectTransformUtility.ScreenPointToWorldPointInRectangle; Overlay uses a null camera, camera rendering uses rootCanvas.worldCamera.
- This replaces direct assignment of screen pixels to world position. It accommodates runtime viewport/render-mode switches made by GamePresentationController without changing item transfer ownership, icon authoring or acquisition presentation.
- `Tools/Validation/DragIconCoordinateRegression.cs` checks screen-projected positions across multiple pointer locations, Overlay/camera/Overlay transitions, an offset camera viewport, parent scaling, and hide/cancel behavior. The actual reported scene still needs visual acceptance.

## KillLock unlocked idle particles (2026-09-21)

- `ChestMonsterKillLockView` projects `ChestMonsterKillLock.IsUnlocked` and the same object's `TreasureChest.IsOpened`; it does not change lock, loot or interaction state. Its optional `unlockedIdleParticle` is an authored child instance. Lock changes and `TreasureChest.WorldStateChanged` refresh the effect; enable/Start resynchronize, and disable unsubscribes and clears particles. Opened-state restoration stops the effect independently of UI-open success.
- `Assets/_Project/Prefabs/VFX/Particle/KillLockUnlockedIdle.prefab` reuses the first existing white Spark sprite with a yellow tint and the URP default Sprite-Unlit material. The production `KillLockTresureChest` and `LowRewardKillLockTreasureChest` variants each contain a nested instance and a view reference. Ordinary chests and legacy `KillLockTreasuteChest` copies are unchanged.
- Accepted preview mapping: 16 pixels per chest unit, emission width 1.25 units (125% of chest width), child local Y -0.06 near its base, start size 0.24609375–0.46265625 (2.25 times the original preview), 2.2-second lifetime, 10 particles/second, maximum 32. Local upward velocity, shrinking and fading create the sustained cue; play-on-awake is off and gameplay pause freezes it. Tune the shared prefab; placement belongs to each chest variant.
- `Tools/Validation/KillLockIdleParticleRegression.cs` checks actual Unity ParticleSystem lifecycle and isolated imports of both wired variants. No runtime object creation, new manager, schema migration or Architecture/Contracts promotion is needed. Main-scene visual acceptance remains a separate check.
