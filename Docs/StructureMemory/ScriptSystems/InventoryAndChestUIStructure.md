...
### Full weapon storage pre-selection rejection

ChestScreen.ToggleSelection rejects adding a WeaponDefinition when all registered WeaponEquip slots are occupied, before mutating selection or moving any slot. It uses the existing blocked-border display for 0.8 unscaled seconds and a weapon-specific discard warning. Deselection is always exempt; after freeing a slot, a new click can select normally. This checks actual occupied storage, not provisional selection reservations or duplicate/trait restrictions; final transfer validation still owns those rules. ChestScreen owns the warning timer and clears it on repeated rejection, successful selection of that slot, or ClearChestSlots (including disable/rebind/reroll).
