...
## Exhibition defaults and opening notice (2026-09-17)

- `Assets/_Project/Runtime/Features/Progression/Upgrades/UpgradeProgressService.cs` owns the nine exhibition preset IDs: out-of-combat travel speed +50%, bag I/II, relic unlock, graves, chest reroll, shop enable/refresh/discount. `GrantExhibitionDefaults` validates all nodes and item-owner readiness, adds only missing purchased IDs without parent purchases, and performs item unlocks through the ready ItemManager. Existing player-effect reapplication and RunModifierService rebuild remain effect owners. Existing eight-node profiles receive the missing travel-speed node on the next grant check without resetting the opening-reward marker.
- `UpgradeProgressSaveService.CheckAndUnlockNodes` grants the preset before resolving normal tree unlocks. New default grants request immediate save even on scene-load checks. This version applies the preset unconditionally; existing purchases are preserved.
- `UpgradeTreeUI.OpenUI` calls `UpgradeManager.NotifyExhibitionUIOpened` only after actual panel activation. Progress owns the per-profile `UpgradeSaveData.exhibitionOpeningRewardGranted` flag; CurrencyManager adds 100 to hub currency. Flag assignment precedes the currency notification/save, preventing reentrant and repeated-open grants. Active-run calls defer an unclaimed reward so it cannot become a pending run delta.
- The existing UpgradeTreePanel prefab authors an `ExhibitionNotice` background and text under the panel root, outside the scrolling tree, below the top navigation arrow. The serialized reference drives visibility after reward confirmation. Both exact user-specified lines remain visible during later openings; closing the panel hides them. No runtime hierarchy creation is used for this notice.
- ChestRunModifierEffect authors five rerolls and its node description matches. Shop refresh remains three and discount 20%. Bag I/II contribute +8 slots; bag III is not included. Previously generated chest runtime state is not rewritten.
- Extension/cleanup: change the narrow preset selection for future exhibition tuning; removing this code does not revoke already persisted purchases. Profile reset starts a new reward lifetime. No new subscriptions, services, singleton, or DDOL owner were added.
- Verification boundary: MSBuild and isolated production-source regression harness passed; Unity import, actual save/restart and in-game layout/rerolls still require Editor verification. Not an Architecture/Contracts promotion candidate.
