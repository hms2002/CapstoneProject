...
- `Assets/_Project/Runtime/Features/Progression/Upgrades/UpgradeProgressService.cs` owns the exhibition preset IDs: bag I/II, relic unlock, graves, chest reroll, shop enable/refresh/discount. `GrantExhibitionDefaults` validates all nodes and item-owner readiness, adds only missing purchased IDs without parent purchases, and performs item unlocks through the ready ItemManager. Existing player-effect reapplication and RunModifierService rebuild remain effect owners.
