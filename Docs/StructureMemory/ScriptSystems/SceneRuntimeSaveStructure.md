...
- Grand Hall is an in-run route checkpoint, not the Hub domain. Its `HubToRunStart` portals start a run only when none is active; during an active run they replace the completed route context while preserving the player snapshot and skipping new-run transition policy. The three normal Boss exit connections return to the arrival-only `GrandHall.BossClear` endpoint with `RunAction.None`.
- Checkpoint availability and warning queries prepare only pending destinations; they retain the committed stage and load-presentation context. `TryActivatePendingPlan` replaces the active route only after destination preparation succeeds, preserving the exact pending selection inspected by the warning check. Clearing the active plan during prompt queries used to leave procedural event generation with no route context. Regression: `SceneConnectionRouteContextPlayModeTests.CheckpointQueries_PreserveCommittedRouteAndEventCandidates_UntilDeparture`.
- Corridor reentry from `ScenePortal` is checked through `RunRoutePlayback.GetTravelBlockWarning`. `PortalRouteManager` examines the prepared first stage for a start portal, or the next stage for `BossToCorridor`, against the active run's defeated boss IDs. It does not use permanent clear progress, so new runs can enter again. `ScenePortal` shows the existing `BossAlreadyDefeatedThisRun` popup before entrance animation/input locking; `ScenePortalTravelCoordinator` repeats the check before route activation, player capture, or travel. Explicit one-shot boss travel overrides retain their existing cheat behavior. Data-driven `SceneConnection` gates and the Demon King portal's separate prerequisite component remain unchanged.
- ScenePortal routes with a non-default entry ID publish that ID as `destinationEndpointId`. `PlayerSpawner` waits for the generated endpoint without requiring a `SceneConnectionSO` ID, allowing Grand Hall route catalogs to place the player at each procedural Start room's `Corridor.<theme>.Lobby` endpoint.
...
- `GamePlayData.affectionGainNpcIds` tracks NPCs that already gained affection during hub preparation and its following run. `TryConsumeAffectionGain` goes through RunSessionStore -> GamePlayDataManager -> RunSessionStateService. StartRun and pending-progress clears preserve this allowance history; active EndRun and development reset clear it. It follows the existing volatile session lifetime, not durable profile saving. See `DialogueNpcAffectionStructure.md` for the affection reward and shop-price projection.
...
- Do not route normal Boss exits to `ProtoTypeHub` or reuse `Lobby.<theme>.Corridor` as their arrival ID. That crosses the real Hub boundary and bypasses the Grand Hall loop. Do not leave a procedural route set's `corridorEntryPointId` at `Default`; it bypasses the generated Start-room endpoint and can spawn the player at an unrelated authored point.
