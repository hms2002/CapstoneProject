...
- `ScenePortal` owns only the pre-travel entrance presentation: it can pull, shrink, and rotate the player before calling `TryTravel(...)`, then the existing travel service still owns route resolution, run state, transition context, and runtime capture. Temporary `PlayerCinematicProtection` and `GameFlowInputBlocker` locks are released before `TryTravel(...)` so presentation/UI block tags are not captured.
