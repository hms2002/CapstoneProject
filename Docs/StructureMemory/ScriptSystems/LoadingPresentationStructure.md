...
- Scene entry music is authored on a scene-only `SceneBgmRequester` root. Boss encounters and GameOver explicitly request their own tracks through Core `SoundPlaybackUtility`; the existing `SoundManager` remains the sole playback owner. Scene-bound requests reject inactive/unloaded source scenes. `RunRouteBgmService`, its Hub fallback, and `RunRouteBgmPlayback` were removed; music no longer depends on portals or run-route state. See [Scene Music Requests](../SceneMusicRequests.md) for authoring and lifecycle details.
