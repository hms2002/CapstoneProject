...
- HP50 `WallBounceRush` owns per-rush `DemonChargeEffect` loop visuals and endpoint `DemonChargeEffectDisapear` one-shots. EgoSword throw/recall owns `SwordSpin4Frame` as a position-following visual that intentionally does not inherit the sword's rotation.
...
- `DarkLord_Sword_Slash` keeps its third/last sprite visible for at least `1` second inside the clip, before any additional code-driven end-pose hold.
- DashStab has a three-beat body rhythm per attack: `DarkLord_Sword_DashStabReady` during warning/prep, `DarkLord_Sword_DashStab` during each rush movement, then `DarkLord_Sword_DashStabReady` again for the recover/end pose hold. PierceCombo uses this rhythm for all three attacks instead of treating DashStab as one clip start per pattern.
...
- HomingMagic uses `DarkLord_Hand_Balt` frame control directly: first frame for projectile creation, last frame held through the fire/dash follow-through.
- HP50 `WallBounceRush` uses linear rush movement to the raycast-derived wall endpoint, pauses at least `0.1` seconds at each endpoint, then recalculates the next rush direction from the current target.
...
- `EgoSwordActor` held state remains hidden body-animation-only, but throw/recall tuning is now Inspector-facing through local throw origin offset, local recall target offset, and initial throw/recall rotation fields. Defaults preserve the previous `(0.85, 0.1, 0)` hold-side position.
...
- DemonKing 50% `WallBounceRush` must run with `SwordMode == Drop`. If HP crosses the 50% gate while the sword is held, pattern selection must force `ThrowEgoSword` first and defer `WallBounceRush` until after the throw commits.
