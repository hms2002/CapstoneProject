...
- Keep `RunRouteCatalog.asset` authored as three normal boss RouteSets (`Shadow`, `Dragon Spili`, `Slime Queen`) plus `DemonkingRouteSet` as final. Runtime hub-start route generation can randomize normal order, but direct boss-scene Play seeds the full catalog deterministically and sets `currentStageIndex` to the tested RouteSet so normal bosses do not look like final-route contexts.
...
- Unity SpriteMask targeting is sorting-range based, not per-mask. If a scene has `GlobalVisionMaskRoot`, a portal renderer using `VisibleInsideMask` for its local gate reveal can also be clipped by the global player vision mask. Keep portal reveal isolation enabled in those scenes, and verify the dark overlay cutout plus portal reveal together in Play Mode.
