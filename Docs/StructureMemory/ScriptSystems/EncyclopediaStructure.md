...
- `EncyclopediaItemLeftPage` is the authored Item LeftPage presenter for title text/icon, item sub-tab buttons, item sub-tab icon authoring references, pagination buttons/text, entry count, and list notices. It relays sub-tab/page requests to `EncyclopediaItemTab` instead of owning selection state.
...
- `EncyclopediaItemRightPage` is the authored Item RightPage presenter. It binds common `Icon` / `Name` fields plus type-specific section roots under the authored layout. Weapon entries show story through the shared `StoryText`, a separate weapon stat text box, and pooled `Panel_AbilityBlock_Encyclopedia.prefab` / `WeaponAbilityBlockView` instances under `AbilityContainer` for Skill1/Skill2 rows. Relic entries do not use `StoryText`; they hide the Story/Description section and write formatted effect text built from `RelicLogic.BuildTooltip(...)` only to a dedicated `relicEffectText` under `relicEffectRoot`, then show preview level text through a separate relic level-preview root. Weapon stat root/text and Relic level-preview root/text should be separate authored references; Weapon mode hides preview guide icons, while Relic mode keeps Prev/Next Q/E KeyGlyph guides visible with disabled alpha when movement is not possible. Each normal weapon skill gets its own block instance; the switch guide appears only for a skill whose source object exposes multiple tooltip variants through `IAbilityTooltipVariantProvider`.
...
- Item selection does not animate; it only refreshes the right-side detail through `EncyclopediaItemRightPage`. The presenter clears type-specific sections before rebinding so previous Weapon/Relic/Consumable sections do not remain visible after selection changes. Weapon ability rows are created from the encyclopedia ability-block prefab pool and display Skill1/Skill2 only. Relic Q/E preview changes the encyclopedia preview level from `1..maxLevel` and refreshes the formatted `RelicLogic.BuildTooltip(...)` effect text in the dedicated relic effect section without using inventory-owned relic levels or `StoryText`; `1 / 1` relics still show disabled Prev/Next guide objects.
- While the Item tab content is visible and interactable, the grid can be browsed with movement input. `D` / MoveRight advances in row-major order and moves to the next page after the last visible item on a page, `A` / MoveLeft reverses that flow, and `W` / `S` move within the same column on the current page with top/bottom wrap. Empty slots on the last page are not selectable.
...
- Relic entries come from `ItemDatabase.allRelics` and bind existing relic display/icon data plus per-level effect text from `RelicLogic.BuildTooltip(relic, previewLevel, context)` into the dedicated relic effect section. The current encyclopedia Relic body does not display `RelicDefinition.description`, and `StoryText` stays inactive for relics until an explicit relic story layout is requested.
