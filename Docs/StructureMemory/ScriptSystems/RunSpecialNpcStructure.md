...
- The authored choice presenter accepts clicks and number keys `1`-`9` after the configured guard window. It supports the current three-button panel by activating only visible choice slots and deactivating unused button objects so layout collapses correctly. For the current three-button authoring, either attach `DialogueChoiceKeyGlyph` to each choice button root with a visual `KeyGlyph` child, or place a slot-owned `KeyNav` / `Nav KeyGlyph` / `KeyGlyph` image under the button or slot parent; the presenter binds visible slots to `1`/`2`/`3` and hides glyph images with unused slots.
...
- Current arrival order is fade out, warp to `appearancePoint`, fade in, then move from appearance to `landingPoint` over the authored arrival movement duration. The movement follows a world-Y parabolic arc using `moveToLandingArcHeight` and `4h*t*(1-t)`, while `moveToLandingCurve` controls timing along that arc. With no `appearancePoint`, this collapses to the old one-position teleport.
- The appearance-to-landing movement is an airborne arrival presentation: the landing endpoint still cannot overlap `HoleTrap`, but the `appearancePoint` and intermediate path may cross `HoleTrap` space. While this movement runs, the player body collider is temporarily disabled so `HoleTrap` trigger/stay logic cannot start pitfall during the cinematic.
- Optional `landingStartParticle` authoring uses a local `SpawnedPresentationHook`. It spawns at the resolved `appearancePoint` after fade-in and before the appearance hold / movement to `landingPoint`, so designers can place a one-shot particle at the visible start of the arrival.
...
- Do not use ground-path `HoleTrap` sampling to decide whether same-scene teleport arrival movement should play. The arrival start and path can intentionally cross holes; validate the final landing endpoint and suppress the body collider during the presentation instead.
- The arrival arc offsets the actual player world position upward during the presentation. Tune `moveToLandingArcHeight`, movement duration, and the timing curve in Play Mode for camera readability and map clearance, while keeping final landing safety validation unchanged.
- `landingStartParticle` is a one-shot start-point presentation hook, not a gameplay collision or movement gate. Its prefab should be visual-only and reviewed in Play Mode for sorting, scale, lifetime, and whether it appears at the intended `appearancePoint`.
