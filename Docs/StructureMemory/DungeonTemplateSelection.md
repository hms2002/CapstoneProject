...
- Mandatory identities remain governed by the role phase. Since 2026-09-17, that phase reserves scarce guaranteed-room slots with bounded rollback (4096 reservation visits per topology); it can move a guaranteed identity between compatible nodes before ordinary roles are assigned. Once template search begins, those identities remain pinned. Combat template rollback still redistributes only Combat quotas.
...
## Required Event Recovery (2026-09-17)

- `DungeonGraphLayoutAssembler.Recovery.cs` wraps the strict assembler. Strict success never enters recovery. `allowRecovery: false` is available for strict authoring checks.
- On exhausted topology search, phase 1 caps Event minimum distance at 2. Phase 2 retries with a disposable policy copy with no cycles, Event minimum distance capped at 1, and Event CycleDetour preference relaxed. Non-Event placement constraints, dead-end requirements, all required identities, Boss distance, room count and combat quotas remain enforced.
- Both guaranteed-room reservation and template domains use the same effective placement predicates. All phases use the existing physical connection/collision validation. Room and policy assets are never edited.
- Missing assets, invalid placement definitions and contradictory role budgets fail preflight rather than silently losing content. Recovery is bounded, not proof that arbitrary libraries can generate. Worst-case generation can take three search budgets, entirely during loading.
- `DungeonLayoutResult.RecoveryLevel/RecoveryDescription` report the effective recovery. Runtime logs a warning when a complete recovered result is accepted.
- `DungeonGenerator` implements Core `ISceneEntryReadiness`; finished-but-failed is not ready. `SceneTransitionCoordinator` waits after Start/settle and before revealing/unlocking. Its exceptional IMGUI retry panel owns no gameplay state or persistent objects; the pending reference clears in the iterator's finally block. This guard applies to coordinated scene transitions, not direct Editor scene startup. A prefab-authored failure screen can replace the exceptional panel separately.
- Failed generation cannot overwrite preserved room object state. Event-plan Commit remains after successful build. Retry uses the same run seed and cannot repair a genuinely invalid library; designer repair is still required.
- Tests cover three themes x three stages x three seeds with delivery and alarm together, a forced tree recovery, deterministic results, immutable authoring assets, and failed-entry/retry gating. See the 2026-09-17 SessionLog for verification and remaining build/manual checks.
