...
9. For every room containing a Monster placement, the builder creates a full-bounds rectangular `MonsterRoomArea2D`, a `MonsterSpawnRoomGroup`, and a separate `RoomEncounterEntryTrigger2D` inset one cell from the boundary Walls. Each connected endpoint door owned by that room receives the existing `RoomDoorMonsterKillLock` behavior. The room group also receives the template's progression-aware `MonsterSpawnRoomProfileSO`.
10. The builder realizes each room's Grid-relative object placements under a dedicated generated root. It creates non-monsters directly, but realizes each Monster placement only as a deferred `MonsterSpawnContainer`. On first room entry the group resolves the current Stage through `StageMonsterSetSO`, creates spawn requests for the available anchors, and lets `SceneMonsterSpawnDirector` instantiate and register the selected monsters. The authored per-placement prefab remains a fallback when no valid profile plan is available.
...
- Play Mode `Monster` placements become inactive runtime spawn anchors during the dungeon build. On first entry the room's Spawn Profile selects current-Stage `StageMonsterSetSO` entries, capped by the available anchor count; if the profile cannot create a plan, anchors fall back to their authored prefabs. A linked chest is resolved before spawning and supplied through `MonsterSpawnRequest.LinkedChestKillLock`, so `SceneMonsterSpawnDirector` remains responsible for registration.
...
- Shadow, Dragon, and Slime keep their respective Corridor tile palettes. Combat composition is no longer fixed to theme-specific prefabs: each theme owns small/big `MonsterSpawnRoomProfileSO` assets that resolve the current Stage through Common Melee, Ranged, and Tank `StageMonsterSetSO` assets.
...
- Rectangular Combat samples contain `2/4/4` Monster spawn anchors; each large corner sample contains `8`. Compact and Tall rooms use the theme's small Spawn Profile, while Wide and both large corner rooms use its big Spawn Profile. The profiles resolve Common Melee, Ranged, and Tank `StageMonsterSetSO` entries for the current run stage; authored anchor prefabs are fallback/preview data, not the normal fixed encounter composition.
