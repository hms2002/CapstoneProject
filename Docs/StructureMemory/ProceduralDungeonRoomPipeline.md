...
- `DungeonGraphLayoutAssembler` chooses Start by its positive authoring weight before graph creation. It collects unique directions using `RoomSocketGeometry.IsValid`, ignoring duplicate directions and invalid sockets. Start templates whose opposite sockets cannot align for the straight-corridor embedder are excluded, with an explicit failure if none remain.
...
- A Start cycle is important for compact layouts: connecting four directions does not require three extra independent branches if an existing cycle provides one of the connections. No room-count, boss-distance, cycle/branch limit, Large cap or required-event rule is weakened. Infeasible settings fail instead of silently closing Start sockets or creating extra rooms.
