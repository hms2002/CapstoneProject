...
- Upgrade NPC feature handoff acquires a direct `PlayerUIControlLockBridge` control tag lock from feature selection until `UpgradeManager.ToggleUI()` starts the UI open flow, so the player cannot move during the dialogue-exit wait. Upgrade open fade is then executed by `UpgradeUiOpenFlow`; it blocks unrelated UI input, then opens `UpgradeTreeUI` through the owned push path.
...
- If a feature must wait for dialogue blockers to release before opening its stack UI, cover that wait with a player-control-only handoff lock rather than an external UI input blocker; an external blocker can deadlock the later UI open unless the owner exception is threaded through the open path.
