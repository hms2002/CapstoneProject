...
| P2 | [Upgrade Runtime Boundary Split](./UpgradeRuntimeBoundarySplit.md) | partially-refactored | `UpgradeManager` now delegates purchase transaction policy, purchase success ordering, runtime effect handoff, run-start eligibility/timing, UI open fade/input-blocker execution, lifecycle subscription/run-start guard state, unlock-check save/notification execution, singleton/persistent root adoption, and UI toggle execution to helpers. Generated project files now include the extracted helpers and MSBuild errors-only verification passes. Remaining debt is callback/event/API ownership. | New upgrade effect type, run-start effect bug, upgrade UI blocker/fade issue, upgrade save timing work, or a planned public API/event ownership pass. |
| P2 | [Runtime Presentation Fallback Authoring Split](./RuntimePresentationFallbackAuthoringSplit.md) | partially-refactored | Runtime-created UI/presentation fallback paths now emit development/editor audit warnings and Scene Setup Validator reports the main missing authored reference risks, but build-facing visual templates still need prefab/scene migration. | Loading/cursor/letterbox/status HUD/Boss HUD visual polish, Canvas/raycast/sorting conflict, or adding another runtime-created UI fallback. |
| P2 | [Scene Run State Lifecycle Ownership Split](./SceneRunStateLifecycleOwnershipSplit.md) | partially-refactored | P1 helper/file split is complete, and source-only slices now isolate run start/end mutation, restore confirmation/pending-state consumption, and route-linked boss reward/portal fallback. Generated project files now include these helpers and MSBuild errors-only verification passes. Static portal travel entry, run-session naming, and scene-facing contract migration still need design/review. | Portal/run/save/boss battle-end flow change, new transition type, restore lifecycle edit, or planned scene/prefab reference pass. |
...
| P2 | [BossDrop Responsibility Split](./BossDropResponsibilitySplit.md) | resolved | `BossDrop` source/meta and serialized scene/prefab script references are removed. Boss battle-end now uses route-linked special reward presets, a common prefab catalog, anchors, reward spawners, portal activators, and validator-backed authoring checks. | Reopen only if the boss battle-end validator or Unity import reveals a structural migration issue that cannot be fixed by normal Inspector authoring. |
