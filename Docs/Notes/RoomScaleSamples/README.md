...
새 방은 기존 라이브러리의 `rooms` 목록에 등록하지 않았다. 따라서 현재 플레이의 무작위 후보와 등장 확률에는 영향을 주지 않는다. 라이브러리 등록/정식 도입과 단계별 선택 정책은 후속 작업이다.
...
- `difficultyTier = 2 / 3`은 견본의 목표 단계 표시다. 현재 런타임은 이 필드로 보스 처치 수나 진행 단계를 필터링하지 않는다. 0 기반 진행도 필드와 혼용하지 않는다.
