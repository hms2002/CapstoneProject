...
> 현재 상태 (2026-09-14): 일반 방 24개 등록 완료. 일반 전투방 개수 기준 1단계 100/0/0, 2단계 50/50/0, 3단계 20/40/40의 비율을 적용한다. Large는 별도로 현재 단계 1개다. 실제 전투 밸런스 플레이 검증은 남아 있다.
...
![취룡 추가 6개](Dragon_additional_overview.png)
...
![그림자 추가 6개](Shadow_additional_overview.png)
...
![슬라임 추가 6개](Slime_additional_overview.png)
...
- 모든 방은 Combat / Normal / 보상 잠금 없음 / 기본 웨이브 하나다. 라이브러리 등록 및 difficultyTier를 사용하는 단계별 개수 비율 적용을 완료했다.
...
- 몬스터 수는 견본 초깃값이며, 면적 증가가 반드시 체감 난이도 증가로 이어지지는 않는다. 단계별 구성 적용 후 플레이 밸런스를 확인한다.
